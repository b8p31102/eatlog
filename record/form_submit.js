$(function(){
		...
		$.ajax({
			...
			success: function(result, textStatus, xhr) {
				...
				for(key in obj) {
					console.log(key);
					console.log(obj[key]);
					$('#result').append('<li><input type="radio" name="eiyouka_id" value="' + key + '">' + obj[key]['name'] + '</li>');
					//$('#result').append('')
				}
	...
	$('#search-r').on("submit",function(event){//formタグ上でのsubmitイベントを処理
		event.preventDefault(); //formタグの働きによるsubmitを抑制
		var form = $(this);
		// 送信ボタンを取得（後で使う: 二重送信を防止する。）
		var button = form.find('submit_btn');
		...
		$.ajax({
			...
			success: function(result, textStatus, xhr) {
				...
				for(key in obj) {
					console.log(key);
					console.log(obj[key]);
					$('#result').append('<li><input type="radio" name="recipe_id" value="' + key + '">' + obj[key]['title'] + obj[key]['serving_for'] + '</li>');
					//$('#result').append('')
				}
