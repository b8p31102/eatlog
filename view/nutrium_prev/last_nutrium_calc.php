<?php
    ...
    */
    ...
    $select_psql1 = "SELECT * FROM eatlog WHERE user_id='{$user_id}' AND e_date='{$e_date}';";
    ...
    while($nutrium_row = pg_fetch_array($result, NULL, PGSQL_ASSOC)){
        //var_dump($nutrium_row);
        foreach($nutrium_row as $col_name=>$value){
            if($col_name != 'recipe_id' && $col_name != 'name' && $col_name != 'id' && $col_name != 'user_id' && $col_name != 'class' && $col_name != 'eiyouka_id' && $col_name != 'quantity'){
                //echo "$col_name, $value, $quantity_rate\n";
                if(!array_key_exists($col_name,$nutrium_array)){ $nutrium_array[$col_name] = 0; }
                $nutrium_array[$col_name] += $value;
            }
        }
    }
    if(!empty($nutrium_array)) {
    $nutrium_array['sex'] = $sex;
    }
    //var_dump($nutrium_array);
//var_dump($result2);
    //var_dump($result);
    //$t_results = array();
/*
...
*/
    ...
    echo json_encode($nutrium_array, JSON_UNESCAPED_UNICODE);
